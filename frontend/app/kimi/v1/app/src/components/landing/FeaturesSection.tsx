...
const features = [
  {
    num: '01',
    title: 'See it come together as you type',
    desc: 'Your resume takes shape in real time — every keystroke builds the next version of you. No refresh, no surprises, just instant clarity.',
    tag: 'Live Preview',
  },
  {
    num: '02',
    title: 'Guidance at every line',
    desc: 'Get precise, line-level feedback on every section — not a vague score, but specific words you can apply in one click. Like a mentor reading over your shoulder.',
    tag: 'AI Copilot',
  },
  {
    num: '03',
    title: 'Know the terrain before you land',
    desc: 'See exactly how your resume performs against applicant tracking systems before you ever hit submit. Every issue flagged, every fix within reach.',
    tag: 'ATS Radar',
  },
  {
    num: '04',
    title: 'Match the map to the mission',
    desc: 'Paste any job description and instantly see which keywords your resume is missing — ranked by how critical they are for that specific role.',
    tag: 'Mission-Ready',
  },
];
...
function getDotState(dotIndex: number, p: number) {
  ...
  let scaleX = 1;
  if (p > start && p < end) {
    const dist = Math.abs(p - targetProgress) / segmentStep;
    if (dist < 0.3) {
      scaleX = 3.2;
    } else if (dist < 0.5) {
      const t = 1 - ((dist - 0.3) / 0.2);
      scaleX = lerp(1, 3.2, t);
    }
  }
      ...
      >
        <div className="container">
          ...
          >
            ...
            >
              Everything you need for takeoff.
            </motion.h2>
            <motion.p
              className="type-body-lg"
              style={{ color: 'var(--color-ink-muted)', maxWidth: '440px', marginInline: 'auto' }}
              initial={{ opacity: 0, y: 16 }}
              animate={headerInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}
            >
              From your first word to your final export — every tool is built in, without the clutter.
            </motion.p>
          </div>
...
function MotionTextPanel({
  ...
  const pointerEvents = useTransform(progress, (p) =>
    getTextState(index, p).opacity > 0.85 ? 'auto' : 'none'
  );
      ...
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: '#F5F8F7' }}>
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, background: '#fff', padding: '18px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ height: 5, background: '#D4E8E4', borderRadius: 3, width: '92%' }} />
    ...
    return (
      <div style={{ width: '100%', height: '100%', background: 'var(--color-ink)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 28px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 7, zIndex: 1 }}>
          {[
            { label: 'Standard Fonts', pass: true },
            { label: 'Column Structure', pass: true },
            { label: 'File Format', pass: true },
            { label: 'Keyword Density', pass: false },
          ].map(({ label, pass }) => (
            <div
              key={label}
              style={{
                ...
                borderRadius: 8,
                padding: '9px 14px',
                border: `1px solid ${pass ? 'rgba(100,182,172,0.15)' : 'rgba(248,113,113,0.2)'}`,
              }}
            >
